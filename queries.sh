...
echo "$($PSQL "select distinct(name) from games g full join teams on g.winner_id = teams.team_id full join games gop on teams.team_id = gop.opponent_id where (g.year=2014 and g.round='Eighth-Final') or (gop.year=2014 and gop.round='Eighth-Final') order by name;")"
...
echo "$($PSQL "select name from teams where naem like 'Co%'")"
